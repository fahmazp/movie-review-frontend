...
const navigation = {
  ...
  social: [
    {
      name: 'Linkedin',
      href: 'https://linkedin.com/in/fahmaz-ashraf',
      icon: (props) => (
...
export default function Footer() {
  return (
    <footer className="relative isolate overflow-hidden bg-gray-200 dark:bg-[#000000]">
      <div className="mx-auto max-w-7xl overflow-hidden px-6 py-16 lg:px-8">
        <nav aria-label="Footer" className="-mb-6 flex flex-wrap justify-center gap-x-12 gap-y-3 text-sm/6">
          {navigation.main.map((item) => (
            <Link key={item.name} to={item.path} className="text-gray-800 dark:text-gray-400">
              {item.name}
            </Link>
          ))}
        </nav>
